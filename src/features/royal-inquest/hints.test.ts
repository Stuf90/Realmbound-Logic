...
const state={...createInitialInquestState(),placements:{envoy:{row:0,column:1},aldric:{row:1,column:0},cedric:{row:3,column:3},daria:{row:4,column:5},edmund:{row:5,column:2}}};
const hint=getInquestHint(definition,state);
expect(hint).toMatchObject({characterId:'beatrice',position:{row:2,column:4}});
