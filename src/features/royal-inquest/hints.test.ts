...
const state={...createInitialInquestState(),placements:{envoy:{row:0,column:3},aldric:{row:1,column:0},cedric:{row:4,column:2},daria:{row:5,column:5},edmund:{row:3,column:4}}};
const hint=getInquestHint(definition,state);
expect(hint).toMatchObject({characterId:'beatrice',position:{row:2,column:1}});
