...
import { commitHistory, createHistory, redoHistory, undoHistory } from '../../shared/history';
import { loadPuzzle, savePuzzle } from '../../shared/persistence';
import { checkSiegeProgress, createInitialSiegeState, editSiegeCell, getSiegeHint, highgatePassage, isSiegeComplete, ROUTE_MASKS, siegeCounts, type NormalMask, type SiegeState } from './engine';
import '../../app/puzzle.css';

const glyph: Record<number, string> = { 1: '╵', 2: '╴', 4: '╷', 8: '╶', 3: '└', 6: '┌', 12: '┐', 9: '┘', 5: '│', 10: '─' };
  ...
  return <main className="app-shell commission-page">
    ...
    <div className="puzzle-layout app-workspace">
      <section className="board-panel">
        <div className="board-scroll"><div className="siege-wrap">
      ...
      <aside className="dossier">
        <button className="text-button dossier-toggle" aria-expanded={dossierOpen} onClick={() => setDossierOpen((open) => !open)}>{dossierOpen ? '▾ Architect reference' : '▸ Architect reference'}</button>
        <div className={`dossier-content${dossierOpen ? '' : ' collapsed'}`}>
          ...
          {activeTray === 'pieces' ? <section className="tray-content" role="region" aria-label="Road pieces"><p>Choose a road tool and cycle a square, or select an exact orientation.</p><div className="orientation-picker">{ROUTE_MASKS.map((mask) => <button key={mask} onClick={() => setStatus(`Orientation ${glyph[mask]} selected; choose a square, then cycle to match.`)}>{glyph[mask]}</button>)}</div></section> : <section className="rule-list" role="region" aria-label="Architect's rules"><ol><li>One continuous road joins the gates.</li><li>Every road segment has matching neighbours.</li><li>There are no branches, crossings, loops, or disconnected stones.</li><li>Each line matches its numbered total.</li></ol></section>}
        </div>
      </aside>
    </div>
  </main>;
