...
`beside` + `not-beside` BOTH DEAD AGAINST PERMUTATION SOLUTION (SEE
[CHARACTER PLACEMENT](character-placement.cave.md) — EVERY SHIP CASE = ONE): PLACEMENT
RULE FORBID TWO CHARACTER SHARE ROW/COLUMN EVER → MANHATTAN DIST NEVER EXACT 1 →
`beside` NEVER SATISFY, `not-beside` ALWAYS TRIVIAL TRUE, NO CONDITION, NO MATTER
CHAMBER. NO COMBINE WITH SAME/DIFFERENT-CHAMBER FACT MAKE IT MEANINGFUL — OR RESOLVE
"ALWAYS TRUE" BEFORE CHAMBER EVEN CONSIDER. **DO NOT AUTHOR `not-beside` CLUE** — ZERO
INFO. (OLDER VERSION THIS DOC CLAIM `not-beside` STILL MEANINGFUL — WRONG. THE
...
1. **NO-EMPTY-ROOM GLOBAL CONSTRAINT** — "NO ROOM STAY EMPTY" AS PUZZLE-LEVEL FACT,
   HELP SOLVE BUT NOT CLUE PREDICATE ITSELF — AUTHOR-TIME INVARIANT (EVERY CHAMBER GET
   >= 1 OCCUPANT AT SOLUTION), ALREADY ENFORCE BY `validateInquestDefinition` "CHAMBER
   NO OCCUPANT IN SOLUTION" CHECK.
