...
- **NO MORE CHARACTERS THAN ROWS OR COLUMNS.** EVERY CHARACTER NEED UNIQUE ROW + UNIQUE
  ...
  character needs a unique row and column.` `blackwoodKeep` SHIP SIX CHARACTER ON 9x6
  BOARD — CONTENT CHOICE, NOT STRUCTURAL LIMIT. BIGGER BOARD SHIP BIGGER CAST, SMALLER
  BOARD SMALLER CAST.
...
1. DEFINE AT LEAST TWO CHARACTERS (NO MORE THAN BOARD ROWS OR COLUMNS), ONE
   `isVictim: true`, UNIQUE ID, VALID `avatarId`.
...
3. PICK `solution` = FULL ROW/COLUMN PERMUTATION ACROSS CAST, EVERY POSITION LEGAL
   UNBLOCKED CELL.
