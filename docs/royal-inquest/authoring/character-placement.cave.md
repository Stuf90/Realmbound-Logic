...
- `InquestCell.legalCharacterIds?: CharacterId[]` RESTRICT CELL TO SPECIFIC CHARACTERS.
  OMIT = ALLOW ANY. `blackwoodKeep` USE ONLY PIN EACH CHARACTER SOLUTION CELL TO THAT
  ONE CHARACTER — RULE OUT ALTERNATE VALID PLACEMENT ALSO SATISFY EVERY CLUE. NOT
  GENERAL-PURPOSE NARRATIVE DEVICE.
- **NEVER SET `legalCharacterIds` ON CELL THAT ALSO HAVE `propId`.** PROP CELL (CHAIR,
  BENCH, THRONE, ...) MUST ALWAYS READ OPEN TO ANY CHARACTER, NOT SEAT RESERVE ONE.
  MOOT TODAY SINCE EVERY PROP CELL ALSO `blocked` — ALREADY UNUSABLE BY ANYONE — BUT
  TWO FIELDS INDEPENDENT, VALIDATION NOT CURRENT REJECT COMBO. DON'T RELY ON `blocked`
  ALONE MAKE RESTRICTION HARMLESS. AUTHOR PROP CELL AS PLAIN `blocked: true` + `propId`,
  NO `legalCharacterIds`.
