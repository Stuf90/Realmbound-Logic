...
TWO PROP IDEA NOT YET IMPLEMENT — CAPTURE HERE AS FUTURE IDEA, NOT SPEC:
...
1. **DOOR** — BLOCK DECORATIVE PROP, DISTINCT FROM EXIST `dungeon-cage`/`bookshelf`/ETC.
   ...
   ABOVE.
2. **WINDOW EDGE-ONLY RULE** — WINDOW PROP ONLY SIT BOARD OUTER EDGE, SPAN TWO GRID
   CELL. NEED NEW ASSET **+** NEW PLACEMENT CONSTRAINT (EDGE-ADJACENCY CHECK) — ENGINE
   HAVE NO EDGE-ADJACENCY CONCEPT TODAY.
